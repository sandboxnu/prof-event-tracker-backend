...
import { Activity, ActivityCategory, PrismaPromise, SignificanceLevel } from '@prisma/client';
...
export class ActivitiesService {
  ...
  findFilter(userId: number, category: ActivityCategory | undefined, significance: SignificanceLevel | undefined) {
    let result:PrismaPromise<Activity[]>;

    if(!significance) {
      result = this.prisma.activity.findMany({ where: {userId: userId, category: category}})
    }
    else if(!category) {
      result = this.prisma.activity.findMany({ where: {userId: userId, significance: significance}})
    }
    else if(!significance && !category) {
      result = this.prisma.activity.findMany({ where: {userId: userId}})
    }
    else {
      result = this.prisma.activity.findMany({ where: {userId: userId, category: category, significance: significance}})
    }

    return result;
  }

  find(activityCatetory: ActivityCategory | undefined, significanceLevel: SignificanceLevel | undefined) {
